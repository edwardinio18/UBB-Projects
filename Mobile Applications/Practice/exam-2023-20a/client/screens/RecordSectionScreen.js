...
const WorkoutsScreen = () => {
        ...
        const syncOfflineWorkouts = async () => {
            ...
            if (offlineWorkouts && offlineWorkouts.length > 0) {
                offlineWorkouts.forEach(workout => {
                    axios.post('http://192.168.0.17:2320/workout', workout)
                        .then(() => {
    ...
    const loadWorkouts = async () => {
        ...
        try {
            ...
            if (isOffline) {
                ...
                const response = await axios.get('http://192.168.0.17:2320/all');
    ...
    const handleDeleteWorkout = async (id) => {
        try {
            setIsLoading(true);
            await axios.delete(`http://192.168.0.17:2320/workout/${id}`);
